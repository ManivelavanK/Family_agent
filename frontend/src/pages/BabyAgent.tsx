...
export default function BabyAgent() {
  ...
  const fetchContext = async () => {
    if (!token) return;
    try {
      const res = await fetch('http://localhost:8000/orchestrator/context/baby', {
  ...
  return (
    <div className="p-6">
      <h1 className="text-2xl font-bold mb-4">BabyAgent (Baby Context)</h1>
      {errorMsg && <p className="text-red-500 mb-4">{errorMsg}</p>}
      
      <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6 mb-6">
        <h2 className="font-semibold text-lg mb-2">Baby Context</h2>
